
//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------


namespace ASArquiteruraData
{

using System;
    using System.Collections.Generic;
    
public partial class tbr_ecf_tributacao
{

    public int trb_id { get; set; }

    public string ecf_numero_serie { get; set; }

    public string trbecf_codigo { get; set; }

    public string trbecf_situacao { get; set; }

    public Nullable<System.DateTime> trbecf_dt_cadastro { get; set; }

    public Nullable<System.DateTime> trbecf_dt_alteracao { get; set; }



    public virtual tb_ecf tb_ecf { get; set; }

    public virtual tb_tributacao tb_tributacao { get; set; }

}

}
