
//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------


namespace ASArquiteruraData
{

using System;
    using System.Collections.Generic;
    
public partial class tbr_convenio_un
{

    public int cnv_id { get; set; }

    public int uneg_id { get; set; }

    public string convun_situacao { get; set; }

    public Nullable<System.DateTime> convun_dt_cadastro { get; set; }

    public Nullable<System.DateTime> convun_dt_alteracao { get; set; }

    public Nullable<decimal> convun_cpf_alteracao { get; set; }



    public virtual tb_convenio tb_convenio { get; set; }

    public virtual tb_unid_negocio tb_unid_negocio { get; set; }

}

}
